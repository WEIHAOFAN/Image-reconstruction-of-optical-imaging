...
% For details of parameters, check mmclab.m
...
elemprop = element_region(mesh.region,mesh.elements);   % covert from node-wise to element-wise
...
cfg.tend=5e-9;
cfg.tstep=1e-10;
cfg.gpuid=1;   % use GPU if 1

% other simulation parameters
cfg.isreflect=1;   % consider refractive index mismatch
cfg.debuglevel='TP';    % debug flag string
cfg.method='elem';      %  ray-tracing method
cfg.srctype='isotropic';    % source type
...
tic
for wavelength = 1
    for i = 1:size(spos2,1)
        clear flux;
        
        % set optical properties: mua, mus, g (musp = mus*(1-g)), refractive index 
        % medium labels: 0-ambient air, 1-air cavities, 2-scalp, 3-skull,
        %                          4-csf, 5-gray matter, 6-white matter
        if wavelength == 1
            cfg.prop=[0,0,1,1;               %  air
                    0.0040 0.01 0.9 1.4;            %  CSF (true mus)
        ...
        % light direction
        % if source type is isotropic, light direction is not needed
